...
import io.piano.common.auth0.json.mgmt.RulesConfig;
...
public class RulesConfigsEntityTest extends BaseMgmtEntityTest {
    ...
    public void shouldListRules() throws Exception {
        ...
        server.jsonResponse(MockServer.MGMT_RULES_CONFIGS_LIST, 200);
        List<RulesConfig> response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("GET", "/api/v2/rules-configs"));
    ...
    public void shouldDeleteRulesConfig() throws Exception {
        ...
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("DELETE", "/api/v2/rules-configs/1"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Content-Type", "application/json"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Authorization", "Bearer apiToken"));
    }
    ...
    public void shouldUpdateRulesConfig() throws Exception {
        ...
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("PUT", "/api/v2/rules-configs/1"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Content-Type", "application/json"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Authorization", "Bearer apiToken"));
