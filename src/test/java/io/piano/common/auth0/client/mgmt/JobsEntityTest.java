...
import io.piano.common.auth0.client.mgmt.filter.UsersExportFilter;
import io.piano.common.auth0.client.mgmt.filter.UsersImportOptions;
import io.piano.common.auth0.json.mgmt.EmailVerificationIdentity;
import io.piano.common.auth0.json.mgmt.jobs.Job;
import io.piano.common.auth0.json.mgmt.jobs.UsersExportField;
import io.piano.common.auth0.net.Request;
import io.piano.common.auth0.net.multipart.FilePart;
import io.piano.common.auth0.net.multipart.KeyValuePart;
import io.piano.common.auth0.net.multipart.RecordedMultipartRequest;
...
public class JobsEntityTest extends BaseMgmtEntityTest {
    ...
    public void shouldGetJob() throws Exception {
        ...
        server.jsonResponse(MockServer.MGMT_JOB, 200);
        Job response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("GET", "/api/v2/jobs/1"));
    ...
    public void shouldRequestUsersExport() throws Exception {
        ...
        server.jsonResponse(MockServer.MGMT_JOB_POST_USERS_EXPORTS, 200);
        Job response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("POST", "/api/v2/jobs/users-exports"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Content-Type", "application/json"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Authorization", "Bearer apiToken"));
    ...
    public void shouldSendUserAVerificationEmailWithNullClientIdAndEmailVerification() throws Exception {
        ...
        server.jsonResponse(MockServer.MGMT_JOB_POST_VERIFICATION_EMAIL, 200);
        Job response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("POST", "/api/v2/jobs/verification-email"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Content-Type", "application/json"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Authorization", "Bearer apiToken"));
    ...
    public void shouldRequestUsersImport() throws Exception {
        File usersFile = new File(MockServer.MGMT_JOB_POST_USERS_IMPORTS_INPUT);
        Request<Job> request = api.jobs().importUsers("con_123456789", usersFile, null);
        assertThat(request, is(notNullValue()));

        server.jsonResponse(MockServer.MGMT_JOB_POST_USERS_IMPORTS, 200);
        Job response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("POST", "/api/v2/jobs/users-imports"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Authorization", "Bearer apiToken"));
