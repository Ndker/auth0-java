...
import io.piano.common.auth0.json.mgmt.DailyStats;
...
public class StatsEntityTest extends BaseMgmtEntityTest {
    ...
    public void shouldGetActiveUsersCount() throws Exception {
        ...
        server.jsonResponse(MockServer.MGMT_ACTIVE_USERS_COUNT, 200);
        Integer response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("GET", "/api/v2/stats/active-users"));
    ...
    public void shouldGetDailyStats() throws Exception {
        ...
        server.jsonResponse(MockServer.MGMT_DAILY_STATS_LIST, 200);
        List<DailyStats> response = request.execute();
        RecordedRequest recordedRequest = server.takeRequest();

        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasMethodAndPath("GET", "/api/v2/stats/daily"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Content-Type", "application/json"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasHeader("Authorization", "Bearer apiToken"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasQueryParameter("from", "20161011"));
        MatcherAssert.assertThat(recordedRequest, RecordedRequestMatcher.hasQueryParameter("to", "20161012"));
